...
class DailyTimeRecord_model extends CORE_Model {
    ...
    function get_pagibig_report($filter){
        $query = $this->db->query("SELECT 
                    daily_time_record.employee_id,
            ...
            ORDER BY refpayperiod.month_id, employee_list.last_name ASC
        ");
        $query->result();
        return $query->result();
    }
    ...
    function get_philhealth_report($filter){
        $query = $this->db->query("SELECT 
                    y.*,
                    (SELECT 
                            (CASE
                                    WHEN y.total_reg_pay <= 10000 THEN rpc.employee
                                    WHEN y.total_reg_pay >= 40000 THEN rpc.employee
                                    ELSE ((y.total_reg_pay * percentage) / 2)
                                END) AS employee
                        FROM
                    ...
                    (SELECT 
                        (CASE
                                WHEN y.total_reg_pay <= 10000 THEN rpc.employer
                                WHEN y.total_reg_pay >= 40000 THEN rpc.employer
                                ELSE ((y.total_reg_pay * percentage) / 2)
                        END) AS employer
                ...
                FROM
                    ...
                    FROM
                        (SELECT 
                        -- SUM(reg_pay) AS total_reg_pay,
                            (CASE
                                WHEN emp_rates_duties.philhealth_salary_credit = 0 || null 
                                    THEN SUM(reg_pay)
                    ...
                    WHERE
                        $filter
                    GROUP BY daily_time_record.employee_id, refpayperiod.month_id, EXTRACT(YEAR FROM refpayperiod.pay_period_end)
                    ORDER BY refpayperiod.month_id, employee_list.last_name ASC) AS x) AS y
                ");
        $query->result();
        return $query->result();
    }
     ...
     function get_sss_report($filter){
        $query = $this->db->query("SELECT 
                        ...
                        (SELECT 
                            ...
                            WHERE
                                y.total_reg_pay BETWEEN rsc.salary_range_from AND rsc.salary_range_to) AS employee,
                        (SELECT 
                                rsc.employer
                            FROM
                                ref_sss_contribution rsc
                            WHERE
                                y.total_reg_pay BETWEEN rsc.salary_range_from AND rsc.salary_range_to) AS employer,
                        (SELECT 
                                rsc.employer_contribution
                            FROM
                                ref_sss_contribution rsc
                            WHERE
                                y.total_reg_pay BETWEEN rsc.salary_range_from AND rsc.salary_range_to) AS employer_contribution,
                        (SELECT 
                                rsc.sub_total
                            FROM
                                ref_sss_contribution rsc
                            WHERE
                                y.total_reg_pay BETWEEN rsc.salary_range_from AND rsc.salary_range_to) AS sub_total,
                        (SELECT 
                                rsc.total
                            FROM
                                ref_sss_contribution rsc
                            WHERE
                                y.total_reg_pay BETWEEN rsc.salary_range_from AND rsc.salary_range_to) AS total
                    FROM
                        (SELECT 
                            x.total_reg_pay,
                                x.employee_id,
                        ...
                        FROM
                            (SELECT 
                                -- SUM(reg_pay) AS total_reg_pay,
                                (CASE
                                    WHEN emp_rates_duties.sss_phic_salary_credit = 0 || null 
                                        THEN SUM(reg_pay)
        ...
        //echo count($dtr_id);
                foreach($dtr_id as $dtr)
                            ...
                            foreach ($regearningstemp->result() as $row)
                            {
                                ...
                                $regulardeductionloans = $this->db->query('SELECT deduction_cycle,new_deductions_regular.deduction_regular_id,deduction_per_pay_amount,daily_time_record.dtr_id,is_deduct,new_deductions_regular.deduction_id,daily_time_record.pay_period_id FROM daily_time_record
                                                                    LEFT JOIN dtr_deductions
                                ...
                                foreach ($regulardeductionloans->result() as $row)
                                {
                                        ...
                                        $chck = $this->db->query("SELECT * FROM reg_deduction_cycle WHERE deduction_regular_id = ".$deduction_regular_id." AND pay_period_id=".$pay_period_id);
                                        $chck_payperiod = $chck->result();
