...
class PayrollReports_model extends CORE_Model {
   ...
   function get_1601C_list($year,$month){
         ...
         //                                        AND EXTRACT(YEAR FROM refpayperiod.pay_period_end) = $year
         //                                        AND deduction_id = 1
         ...
         //                        AND EXTRACT(YEAR FROM payperiod.pay_period_end) = $year
         ...
         // return $query->result();
            $query = $this->db->query("SELECT
                        ...
                        ORDER BY emp.last_name ASC) AS A");
            return $query->result();
    }
    ...
    function get_payroll_register($department,$branch,$pay_period,$type){
        $query = $this->db->query('SELECT ps.*,CONCAT(el.first_name," ",el.middle_name," ",el.last_name) as fullname,rpp.pay_period_start,rpp.pay_period_end,el.*,
    ...
    function getloanamount($filter_value,$filter_value2){
        $loan_temp = $this->db->query('SELECT new_deductions_regular.deduction_regular_id,new_deductions_regular.loan_total_amount,new_deductions_regular.deduction_total_amount,COALESCE((psd.deduction_amount),0) as psdamount,new_deductions_regular.deduction_per_pay_amount,COUNT(new_deductions_regular.deduction_regular_id) as count FROM new_deductions_regular
         ...
         function get_13thmonthpay_wofilter($filter_value3) {
            //1=allowance 2=salary adjustments 3=meal allowance 4=other allowance 5=commision
            $query = $this->db->query('SELECT ps.*,CONCAT(el.last_name,", ",el.first_name," ",el.middle_name) as fullname, SUM(ps.reg_pay) as total_reg,erd.salary_reg_rates,SUM(erd.salary_reg_rates) as total_basic_pay,dtr.for_13th_month,SUM(dtr.for_13th_month) as total_13thmonth,SUM(dtr.reg_amt) as total_reg_amt,
    ...
    function get_13thmonthpay_year_filter($filter_value2,$filter_value3) {
            ...
            $query = $this->db->query('SELECT ps.*,CONCAT(el.last_name,", ",el.first_name," ",el.middle_name) as fullname, SUM(ps.reg_pay) as total_reg,erd.salary_reg_rates,SUM(erd.salary_reg_rates) as total_basic_pay,dtr.for_13th_month,SUM(dtr.for_13th_month) as total_13thmonth,SUM(dtr.reg_amt) as total_reg_amt,
                                        ...
                                        LEFT JOIN (SELECT pay_slip_id,(earnings_amount) as earnings_amount FROM pay_slip_other_earnings WHERE earnings_id=7) as salary_retro
                                            ON ps.pay_slip_id=salary_retro.pay_slip_id
                                        WHERE extract(YEAR from rpp.pay_period_start)='.$filter_value2.' 
                                            AND el.is_deleted = 0
                                            AND erd.active_rates_duties=1 
    ...
    function get_13thmonthpay_employee_filter($filter_value,$filter_value3) {
            //1=allowance 2=salary adjustments 3=meal allowance 4=other allowance 5=commision
            $query = $this->db->query('SELECT ps.*,CONCAT(el.last_name,", ",el.first_name," ",el.middle_name) as fullname, SUM(ps.reg_pay) as total_reg,erd.salary_reg_rates,SUM(dtr.reg_amt) as total_reg_amt,SUM(erd.salary_reg_rates) as total_basic_pay,dtr.for_13th_month,SUM(dtr.for_13th_month) as total_13thmonth,
                                         SUM(ps.days_wout_pay_amt) as total_days_wout_pay_amt,  SUM(ps.minutes_late_amt) as total_minutes_late_amt,COALESCE(SUM(salary_retro.earnings_amount),0) as retro
                                         FROM pay_slip as ps
                                        LEFT JOIN daily_time_record as dtr
                                            ON ps.dtr_id=dtr.dtr_id
    ...
    function get_13thmonthpay($filter_value,$filter_value2,$filter_value3) {
            // 1=allowance 2=salary adjustments 3=meal allowance 4=other allowance 5=commision
            $query = $this->db->query('SELECT ps.*,CONCAT(el.first_name," ",el.middle_name," ",el.last_name) as fullname, SUM(ps.reg_pay) as total_reg,
                SUM(dtr.reg_amt) as total_reg_amt,
                erd.salary_reg_rates,SUM(erd.salary_reg_rates) as total_basic_pay,
                dtr.for_13th_month,SUM(dtr.for_13th_month) as total_13thmonth,
                                         SUM(ps.days_wout_pay_amt) as total_days_wout_pay_amt, SUM(ps.minutes_late_amt) as total_minutes_late_amt,COALESCE(SUM(salary_retro.earnings_amount),0) as retro
                                         FROM pay_slip as ps
                                        LEFT JOIN daily_time_record as dtr
                                            ON ps.dtr_id=dtr.dtr_id
                                        ...
                                        WHERE dtr.employee_id='.$filter_value.' 
                                            AND el.is_deleted = 0
                                            AND extract(YEAR from rpp.pay_period_start)='.$filter_value2.' 
                                            AND erd.active_rates_duties=1 
                                            AND erd.ref_branch_id='.$filter_value3.'
    ...
    function get_employee_compensation($filter_value,$filter_value2) {
            ...
            $query = $this->db->query("SELECT m.*,SUM(m.reg_pay) as sum_regpay,SUM(m.net_pay) as sum_netpay,
                                       ...
                                       FROM(SELECT
                                          ...
                                          ROUND(SUM(for_13th_month)/11,2) as t3rthmonth,ROUND(SUM(total_deductions),2) as total_deductions
                                     FROM pay_slip
            ...
            $query = $this->db->query("SELECT
                                    ...
                                    FROM
                                        employee_list
                                            LEFT JOIN
                                        (SELECT
                                            ((SUM(daily_time_record.for_13th_month)-(SUM(pay_slip.days_wout_pay_amt)))/11) as acc_13thmonth_pay,
                                        ...
                                        WHERE employee_list.is_deleted = 0
