                        ...
                        COALESCE((SELECT 
                            ...
                            WHERE
                                ndr.employee_id = $employee_id
                                    ...
                                    AND ndr.deduction_id = m.deduction_id),0) AS dr_id
                FROM
                    (SELECT 
                    refdeduction.*,
        ...
        $query = $this->db->query("SELECT z.* FROM (SELECT x.*, 
                (SELECT 
                    SUM(psd1.deduction_amount)
                        ...
                        AND rpp1.pay_period_year = x.pay_period_year) AS total_pagibig_deduction
            FROM 
            (SELECT 
                    refpayperiod.month_id,
                    ...
                    employee_list.ecode,
                    ref_branch.branch,
                    (SELECT pagibig1 FROM reffactorfile) as employee,
                    (CASE
     ...
     function get_sss_report($filter,$month_id){
        $query = $this->db->query("SELECT 
                    ...
                    FROM
                        (SELECT 
                            y.*,
                                (SELECT rsc.employee FROM ref_sss_contribution rsc WHERE y.total_reg_pay BETWEEN rsc.salary_range_from AND rsc.salary_range_to) AS employee,
                                (SELECT rsc.employer FROM ref_sss_contribution rsc WHERE y.total_reg_pay BETWEEN rsc.salary_range_from AND rsc.salary_range_to) AS employer,
                                (SELECT rsc.employer_contribution FROM ref_sss_contribution rsc
                                    WHERE y.total_reg_pay BETWEEN rsc.salary_range_from AND rsc.salary_range_to) AS employer_contribution,
                                (SELECT rsc.sub_total FROM ref_sss_contribution rsc WHERE y.total_reg_pay BETWEEN rsc.salary_range_from AND rsc.salary_range_to) AS sub_total,
                                (SELECT rsc.total FROM ref_sss_contribution rsc WHERE y.total_reg_pay BETWEEN rsc.salary_range_from AND rsc.salary_range_to) AS total
                        FROM
                            (SELECT 
                            x.pay_slip_id,
